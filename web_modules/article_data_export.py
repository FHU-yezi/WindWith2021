...
from pywebio.output import (clear, put_button, put_link, put_text, toast,
                            use_scope)
...
def ExportArticleData(format: str) -> None:
    ...
    try:
        ...
        # True / False => 是 / 否
        articles_data["is_top"] = articles_data["is_top"].apply(lambda x: "是" if x else "否")
        articles_data["paid"] = articles_data["paid"].apply(lambda x: "是" if x else "否")
        articles_data["commentable"] = articles_data["commentable"].apply(lambda x: "是" if x else "否")

        # 更改文章数据的列名
        articles_data.rename(columns={
            "aid": "文章 ID",
            "title": "标题",
            "aslug": "文章 Slug",
            "release_time": "发布时间",
